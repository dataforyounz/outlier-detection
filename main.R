...
x <- sample(x)
find_ueda_outliers(x, smax = NULL )

ueda_outliers( x = x, s_max = 5 )
iqr_outliers( x = x )
z_outliers( x = x )


cbind( data = sort(x), 
       ueda_flag = ueda_outliers( x = x, s_max = 5 )$flag, 
       iqr_flag = iqr_outliers( x = x )$flag,
       zs_flag = z_outliers( x = x )$flag )
